...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Battlesnake Rainbow DQN v3 with Self-Play")
    parser.add_argument(
        "--mode",
        type=str,
        default="duel",
    ...
    logger = setup_logger(log_dir=args.log_dir, log_name="rainbow_train_v3")
    ...
    gui = BoardGUI(title=f"Battlesnake Rainbow v3 — {args.mode}") if args.gui else None
    ...
    def save_policy(tag: str, *, include_training_state: bool = True) -> None:
        path = os.path.join(ckpt_dir, f"rainbow_v3_{run_id}_{tag}.pt")
        save_checkpoint(
            path,
            policy,
            build_meta(),
    ...
    def save_best() -> None:
        tagged = os.path.join(ckpt_dir, f"rainbow_v3_{run_id}_best.pt")
        save_checkpoint(
            tagged,
            policy,
            build_meta(),
        ...
        alias = os.path.join(ckpt_dir, "rainbow_v3_best.pt")
        save_checkpoint(
            alias,
            policy,
            build_meta(),
